...
class Server(object):
    ...
    def connection_thread(self):
        ...
        while True:
            ...
            if type(request) == int and request == client_id['user']:
                ...
                print('LOBBY USERS')
                print(self.lobby_users)
                print(self.lobby_games)
                for game in self.lobby_games:
                    if int(request[1]) == game.id and game.not_full():
                        for user in self.lobby_users:
                            print(user.username + ' ' + request[2])
                            if user.username == request[2]:
                                print('added')
    ...
    def server_input_thread(self, user: Server_User):
        ...
        while user.connected:
            ...
            if type(rx_data) == int and rx_data == conn_id['disconnect']:
                ...
                self.initialize_game(self.assign_id(), rx_data)
    ...
    def authenticate(self, connection: socket, address: str):
        ...
        user = Server_User(rx_data[0], int(
            account_balance), connection, address)

        self.connect_user(user)

        t = threading.Thread(target=self.server_input_thread, args=(user,))
        t.name = rx_data[0] + '_server_input_thread'
        t.start()
    ...
    def disconnect_user(self, user: Server_User):
        ...
        print('-' * 5 + user.username + ' Disconnected ' + 5 * '-')

    # Games Management

    def initialize_game(self, game_id, game_properties):

        game = Poker_Game(game_id, game_properties)
    ...
    def update_lobby_view(self):
        ...
        system_games = []
        for game in self.lobby_games:
            system_games += [game.id, game.name, game.players,
                             game.players_limit, game.start_time]
        ...
        for user in self.lobby_users:
            ...
            aux = lobby_view_update + [str(user.account_balance)]
            print(aux)
            user.send(aux)
        time.sleep(0.1)

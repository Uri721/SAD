...
class Game_SetUp(object):
    ...
    def gameSetUp(self):
        ...
        def process_input():
            ...
            self.stup = [self.tipusJoc, nomJoc, timeStart, numMaxJugadors, dinersInicials,
                         stRatio, self.rbuy, st]
        ...
        rebuy = tkinter.Label(
            fra, text="Rebuy", borderwidth=10, font="Helvetica 15")
        rebuy.grid(sticky='W')

        optList = ["Yes", "No"]
        vari = tkinter.StringVar(fra)
        vari.set(optList[0])

        op = tkinter.OptionMenu(fra, vari, *optList, command=gTypeRebuy)
        op.config(width=18, font="Helvetica 15")
